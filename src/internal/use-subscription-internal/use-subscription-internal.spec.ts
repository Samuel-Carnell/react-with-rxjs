...
type useSubscriptionParams = Parameters<typeof useSubscriptionInternal>;
type useSubscriptionReturn = ReturnType<typeof useSubscriptionInternal>;

function renderUseSubscriptionInternalHook(
	...
	return renderHook((params: useSubscriptionParams) => useSubscriptionInternal(...params), {
		initialProps: initialParams,
	});
}

describe('useSubscriptionInternal', () => {
	it('calls the subscriptionFactory', () => {
		const mockSubscriptionFactory = jest.fn().mockReturnValue(new Subscription());
		renderUseSubscriptionInternalHook([mockSubscriptionFactory, []]);
	...
	it('calls the subscriptionFactory callback once when called with the same dependency array multiple times', () => {
		...
		const mockSubscriptionFactory = jest.fn().mockReturnValue(new Subscription());
		const { rerender } = renderUseSubscriptionInternalHook([mockSubscriptionFactory, dependencies]);
	...
	it('calls the subscriptionFactory callback once when called multiple times with a new dependency array containing the same object reference', () => {
		const object = {};
		const mockSubscriptionFactory = jest.fn().mockReturnValue(new Subscription());
		const { rerender } = renderUseSubscriptionInternalHook([mockSubscriptionFactory, [object]]);
	...
	it('calls the subscriptionFactory callback once when called multiple times with a new dependency array containing the same value', () => {
		const mockSubscriptionFactory = jest.fn().mockReturnValue(new Subscription());
		const { rerender } = renderUseSubscriptionInternalHook([mockSubscriptionFactory, [1]]);
	...
	it('recalls the subscriptionFactory when the dependencies change', () => {
		const mockSubscriptionFactory = jest.fn().mockReturnValue(new Subscription());
		const { rerender } = renderUseSubscriptionInternalHook([mockSubscriptionFactory, ['a']]);
	...
	it('calls unsubscribe on the previous subscription when the dependencies change', () => {
		...
		const { rerender } = renderUseSubscriptionInternalHook([
	...
	it('calls the subscriptionFactory twice when called twice with [{ a: 1 }], then [{ a: 1 }]', () => {
		const mockSubscriptionFactory = jest.fn().mockReturnValue(new Subscription());
		const { rerender } = renderUseSubscriptionInternalHook([mockSubscriptionFactory, [{ a: 1 }]]);
	...
	it('calls unsubscribe on the returned subscription when the hook is unmounted', async () => {
		...
		const mockSubscriptionFactory = jest.fn().mockReturnValue(mockSubscription);
		const { unmount, waitFor } = renderUseSubscriptionInternalHook([mockSubscriptionFactory, []]);
		unmount();
